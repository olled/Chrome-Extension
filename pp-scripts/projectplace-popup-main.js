...
/*global window,mainViewConfig, clearBadgeText, chrome,  toLocaleDateString, $, ich, Spinner,
 singleViewActions, console, sort_by*/
...
var views = {
	...
	mainView: function (trendingBy) {
		...
		if (trendingBy == mainViewConfig.TRENDINGBYCOMMENTS) {
			trendingId = user.getTrendingConversationOnCommentsId();
			...
			trendingId = user.getTrendingConversationOnLikesId();
			user.setTrendingConversationObject(trendingId);	
	...
	displayCommentsForSingleConversation:  function (t, xhr) {
	...
	singleConversationsView: function (conversationsId)
	{
		...
		function _presentConversation(conversationObject) {
			...
			user.getSpecificConversationComments(conversationsId, views.displayCommentsForSingleConversation);	
		}
	...
	allPeoplesView: function () {
		...
		for (var j = 0; j < allImages.length; j++) {
			...
			if (allImages[j].parentNode.id) {
				user.getUserImageHref(allImages[j].parentNode.id, function (sHref) {
					allImages[j].src = sHref;
				});
			}
		}
		$('#searchUsers').focus();
...
var singleActionOnObject = {
	postCommentOnConversation: function (triggerObject, targetId, messageContainer) {
		var postMessage = $('#' + messageContainer).val();
		user.postConversationComment(targetId, postMessage, function (txt, xhr) {
			triggerObject.disabled = '';
			$('#' + messageContainer).val('');
			user.getSpecificConversationComments(targetId, views.displayCommentsForSingleConversation);
		});
	}
